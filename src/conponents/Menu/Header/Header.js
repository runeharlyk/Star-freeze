...
const Header = () => {
    ...
    const [albumPlay] = useSound(GallerySound);
    const [eshopPlay] = useSound(EshopSound);
    const [chatPlay] = useSound(ChatSound);
    ...
    return <div className={styles.main}>
        <div className={styles.container}>
            ...
            <Navigatablechild x="3" y="0"><Fab onClick={albumPlay} className={styles.blue}><PhotoLibrary /></Fab></Navigatablechild>
        </div>
        <div className={styles.container}>
            <div>{time}</div>
            <div><Wifi /></div>
